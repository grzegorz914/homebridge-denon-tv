...
class MainZone extends EventEmitter {
    constructor(api, device, zone, name, host, port, generation, devInfoFile, inputsFile, inputsNamesFile, inputsTargetVisibilityFile, refreshInterval) {
        ...
        this.reference = '';
        this.volume = 0;
        this.volumeDisplayType = 'Relative';
        this.mute = true;
                ...
                this.televisionService.getCharacteristic(Characteristic.PictureMode)
                    ...
                    .onSet(async (command) => {
                        try {
                            ...
                            await this.denon.send(command);
                            const info = this.disableLogInfo ? false : this.emit('info', `set Picture Mode: ${PictureModesDenonString[command]}`);
                        } catch (error) {
    ...
    async start() {
        try {
            ...
            this.denon.on('deviceInfo', (manufacturer, modelName, serialNumber, firmwareRevision, deviceZones, apiVersion, supportPictureMode) => {
                ...
                .on('stateChanged', async (power, reference, volume, volumeDisplayType, mute, pictureMode) => {
                    const input = this.inputsConfigured.find(input => input.reference === reference) ?? false;
                    const inputIdentifier = input ? input.identifier : this.inputIdentifier;
                    const scaledVolume = await this.scaleValue(volume, volumeDisplayType === 'Relative' ? -80 : 0, volumeDisplayType === 'Relative' ? 18 : 98, 0, this.volumeMax);
                    ...
                    if (!this.disableLogInfo) {
                        ...
                        this.emit('info', `Volume: ${volume}${volumeDisplayType === 'Relative' ? 'dB' : '%'}`);
